...
# Matrices ----have to have the same type of values (like vectors)
...
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #submatrix 2 - 4 row and 1 - 3 columns
...
# Lists ---- we can mix data types in lists. assign names to values in the list. 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]]
#the two brackets will give you the ??
...
> Answer: if it has one value it goes into the values and if it has more than one it goes into data

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: It shows what columns are in that dataframe
...
```{r}
high_temp <- c(50, 53, 50, 56, 54, 51, 52, 40, 37, 35)
  
low_temp <- c(40, 31, 29, 27, 40, 30, 33, 25, 20, 22)
...
high_cel <- Cel_func(high_temp)

low_cel <- Cel_func(low_temp)
...
```{r}
temps_df <- data.frame(high_temp, low_temp, high_cel, low_cel)
...
#Functions ymd(), mdy(), ydm() do not take argument cutoff_2000 - what does this mean??
```
